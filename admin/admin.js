(function () {
  ...
  $("verifyBtn").addEventListener("click", async function () {
    ...
    try {
      const data = await __CEAuth.fetch("/api/license/activate", { method: "POST", body: JSON.stringify({ license: txt }) });
      if (data.ok) {
        const p = data.payload;
        setStatus($("verifyResult"), "VÁLIDA. " + p.email + " / " + p.whatsapp + ". Días restantes: " + data.daysLeft + ".", "ok");
      } else {
        setStatus($("verifyResult"), "INVÁLIDA: " + (data.error || "desconocido"), "err");
      }
    } catch (e) {
      setStatus($("verifyResult"), "Error: " + e.message, "err");
    }
  });
